...
/** Pitching team abbrev for a BIP (defense). */
function pitchingTeamAbbrev(hit: VenueHit): string | null {
  if (hit.halfInning === "top") return hit.homeAbbrev ?? null;
  if (hit.halfInning === "bottom") return hit.awayAbbrev ?? null;
...
function upsertIndexEntry(season: number, entry: PlayerBipIndexEntry, generatedAt: string): void {
  const existing = loadPlayerPitchBipIndex(season);
...
export function loadPlayerPitchBipIndex(season: number): PlayerBipIndex | null {
  return readJson<PlayerBipIndex>(indexPath(season));
}

export function loadPlayerPitchBipDetail(season: number, playerId: number): PlayerBipDetail | null {
  return readJson<PlayerBipDetail>(playerPath(season, playerId));
}

export function searchPlayerPitchBipIndex(
  season: number,
  query: string,
  limit = 10,
): PlayerBipIndexEntry[] {
  const index = loadPlayerPitchBipIndex(season);
...
/**
 * Incrementally merge new venue hits into per-pitcher BIP-allowed files + index.
 */
export function appendHitsToPlayerPitchBipStore(
  season: number,
  venueId: number,
  hits: VenueHit[],
  ...
  for (const raw of hits) {
    ...
    const pitcherId = hit.pitcherId;
    if (!pitcherId) continue;
    const list = byPlayer.get(pitcherId) ?? [];
    list.push(hit);
    byPlayer.set(pitcherId, list);
  }
  ...
  for (const [playerId, incoming] of byPlayer) {
    const existing = loadPlayerPitchBipDetail(season, playerId);
    ...
    const name =
      incoming.find((h) => h.pitcherName)?.pitcherName ||
      existing?.name ||
      `Player ${playerId}`;
...
/** Rebuild player-pitch-bip index + per-pitcher files from ballpark-hits venue JSON. */
export function rebuildPlayerPitchBipStore(season: number): {
  playerCount: number;
  bipCount: number;
} {
  ...
  if (existsSync(venueDir)) {
    for (const file of readdirSync(venueDir)) {
      ...
      for (const raw of detail.hits) {
        ...
        let bucket = byPlayer.get(pitcherId);
        if (!bucket) {
          bucket = {
...
/** Merge batter + pitcher search indexes, deduping by playerId with role badges. */
export function mergePlayerSearchIndexes(
  batters: PlayerBipIndexEntry[],
  pitchers: PlayerBipIndexEntry[],
  limit = 10,
): PlayerBipIndexEntry[] {
  const byId = new Map<number, PlayerBipIndexEntry>();
  ...
  for (const entry of pitchers) {
    ...
    const roles = new Set(existing.roles ?? ["batter"]);
    roles.add("pitcher");
    byId.set(entry.playerId, {
      ...existing,
      // Prefer higher bip activity for sort tie-break display counts.
      bipCount: Math.max(existing.bipCount, entry.bipCount),
      hitCount: Math.max(existing.hitCount, entry.hitCount),
      venueCount: Math.max(existing.venueCount, entry.venueCount),
      teamAbbrev: existing.teamAbbrev ?? entry.teamAbbrev,
      teamId: existing.teamId ?? entry.teamId,
      roles: [...roles] as PlayerProfileRole[],
    });
  }

  return [...byId.values()]
    .sort((a, b) => b.bipCount - a.bipCount || a.name.localeCompare(b.name))
    .slice(0, limit);
}
